...
/**
 * Test to check expression building.
 */
...
int main( void ) {
	...
	assert( errcode == 0 );
	enum HilbertExpressionType type = hilbert_expression_add( expr, kind1, &errcode );
	assert( errcode == HILBERT_ERR_INVALID_HANDLE );
	type = hilbert_expression_add( expr, vv, &errcode );
	assert( errcode == 0 );
	...
	expr = hilbert_expression_start( module, &errcode );
	assert( errcode == 0 );
	type = hilbert_expression_add( expr, v1, &errcode );
	assert( errcode == 0 );
	assert( type == HILBERT_FINISHED_EXPRESSION );
	hilbert_expression_free( expr );
	expr = hilbert_expression_start( module, &errcode );
	...
	expr = hilbert_expression_start( module, &errcode );
	assert( errcode == 0 );
	type = hilbert_expression_add( expr, f, &errcode );
	assert( errcode == 0 );
