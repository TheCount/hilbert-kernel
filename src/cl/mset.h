...
#ifndef CL_ModuleSet_H__
#define CL_ModuleSet_H__
 ...
 */
static inline int hilbert_mset_init( ModuleSet * set ) WARN_UNUSED_RESULT;
static inline int hilbert_mset_init( ModuleSet * set ) {
	assert( set != NULL );

	set->count = 0;
	set->threshold = (CL_ModuleSet_MAXLOAD - 1) * CL_ModuleSet_NUMBUCKETS / CL_ModuleSet_MAXLOAD;
	set->sizemask = CL_ModuleSet_NUMBUCKETS - 1;

	set->buckets = calloc(CL_ModuleSet_NUMBUCKETS, sizeof( *set->buckets ) );
	if ( set->buckets == NULL ) {
		return -1;
	} else {
		return 0;
 ...
 */
static inline ModuleSet * hilbert_mset_new(void) WARN_UNUSED_RESULT;
static inline ModuleSet * hilbert_mset_new(void) {
	...
	int rc = hilbert_mset_init( result );
	if ( rc != 0 ) {
		goto nobucketmem;
	}
 ...
 */
static inline void hilbert_mset_fini( ModuleSet * set ) {
	assert( set != NULL );
 ...
 * 	On error, <code>NULL</code> is returned.
 */
static inline ModuleSet * hilbert_mset_clone(const ModuleSet * set) WARN_UNUSED_RESULT;
 ...
 */
static inline struct ModuleSetBucket * hilbert_mset_find(struct ModuleSetBucket * buckets, size_t sizemask, const struct HilbertModule * value, size_t hash) {
	assert (buckets != NULL);
	assert (sizemask > 0);
	assert (sizemask < SIZE_MAX);
	assert ((sizemask & (sizemask + 1)) == 0);
 ...
 */
static inline int hilbert_mset_add(ModuleSet * set, struct HilbertModule * value) WARN_UNUSED_RESULT;
 ...
 * 	On error, <code>-1</code> is returned.
 */
static inline int hilbert_mset_addall(ModuleSet * restrict dest, const ModuleSet * restrict src) WARN_UNUSED_RESULT;
 ...
 */
static inline int hilbert_mset_remove(ModuleSet * set, const struct HilbertModule * value) {
	assert (set != NULL);
 ...
 */
static inline struct HilbertModule * hilbert_mset_get( ModuleSet * set, const struct HilbertModule * value ) {
	assert( set != NULL );

	struct ModuleSetBucket * candidate = hilbert_mset_find(set->buckets, set->sizemask, value, cl_hash_pointer(value));
	assert( candidate->state == CL_ModuleSet_BUCKET_OCCUPIED );
 ...
 * 	Otherwise, <code>0</code> is returned.
 */
static inline int hilbert_mset_contains(const ModuleSet * set, const struct HilbertModule * value) {
	assert (set != NULL);
 ...
 * 	returned.
 */
static inline void * hilbert_mset_iterator_start( const ModuleSet * set ) WARN_UNUSED_RESULT;
static inline void * hilbert_mset_iterator_start( const ModuleSet * set ) {
	assert( set != NULL );
 ...
 */
static inline struct HilbertModule * hilbert_mset_iterator_get( const ModuleSet * set, void * iter ) {
	assert( set != NULL );
	assert( iter != NULL );

	struct ModuleSetBucket * current = iter;
 ...
 */
static inline void * hilbert_mset_iterator_next( const ModuleSet * set, void * iter ) WARN_UNUSED_RESULT;
static inline void * hilbert_mset_iterator_next( const ModuleSet * set, void * iter ) {
	assert( set != NULL );
	assert( iter != NULL );
