...
#ifndef CL_IndexVector_H__
#define CL_IndexVector_H__
...
#undef COMPARATOR
#define CL_IndexVector_IS_SORTABLE
#endif
#undef CL_A
#undef CL_B
 ...
 */
static inline int hilbert_ivector_init2( IndexVector * vector, size_t minsize ) WARN_UNUSED_RESULT;
static inline int hilbert_ivector_init2( IndexVector * vector, size_t minsize ) {
	assert( vector != NULL );
 ...
 * 	On error, -1 is returned.
 */
static inline int hilbert_ivector_init( IndexVector * vector ) WARN_UNUSED_RESULT;
static inline int hilbert_ivector_init( IndexVector * vector ) {
	return hilbert_ivector_init2( vector, 0 );
}
 ...
 */
static inline IndexVector * hilbert_ivector_new2( size_t minsize ) WARN_UNUSED_RESULT;
static inline IndexVector * hilbert_ivector_new2( size_t minsize ) {
	IndexVector * result;
	...
	int rc = hilbert_ivector_init2( result, minsize );
	if ( rc != 0 ) {
		free( result );
		return NULL;
	}
 ...
 * 	On error, <code>NULL</code> is returned.
 */
static inline IndexVector * hilbert_ivector_new(void) WARN_UNUSED_RESULT;
static inline IndexVector * hilbert_ivector_new(void) {
	return hilbert_ivector_new2( 0 );
}
 ...
 * @param vector Pointer to vector.
 */
static inline void hilbert_ivector_fini( IndexVector * vector ) {
	assert( vector != NULL );
 ...
 * 	#hilbert_ivector_fini().
 * 	The returned array must be freed by the caller.
 */
static inline HilbertHandle * hilbert_ivector_dismantle( IndexVector * vector ) WARN_UNUSED_RESULT;
static inline HilbertHandle * hilbert_ivector_dismantle( IndexVector * vector ) {
	assert( vector != NULL );

	HilbertHandle * result = realloc( vector->data, vector->count * sizeof( *vector->data ) );
 ...
 */
static inline int hilbert_ivector_pushback(IndexVector * vector, HilbertHandle elt) WARN_UNUSED_RESULT;
 ...
 * 	If the vector does not have any elements, the behaviour is undefined.
 */
static inline HilbertHandle hilbert_ivector_first( const IndexVector * vector ) {
	assert( vector != NULL );
	assert( vector->count > 0 );
 ...
 */
static inline HilbertHandle hilbert_ivector_last( const IndexVector * vector ) {
	assert( vector != NULL );
	assert( vector->count > 0 );
 ...
 * 	On error, -1 is returned.
 */
static inline int hilbert_ivector_append( IndexVector * restrict dest, const IndexVector * restrict src ) {
	assert( dest != NULL );
	assert( src != NULL );
	...
	if ( newsize < dest->size ) {
		...
		size_t newalloc = newsize * sizeof( HilbertHandle );
		if ( newalloc <= sizeof( HilbertHandle ) ) {
			return -1;
		}
		HilbertHandle * newdata = realloc( dest->data, newalloc );
		if ( newdata == NULL ) {
			return -1;
		}
		dest->size = newsize;
	...
	memcpy( dest->data + dest->count, src->data, src->count * sizeof( HilbertHandle ) );
	dest->count = newcount;
...
static inline HilbertHandle * hilbert_ivector_toarray( const IndexVector * vector ) {
	...
	HilbertHandle * result = malloc( vector->count * sizeof( HilbertHandle ) );
	if ( result == NULL ) {
		return NULL;
	}
	memcpy( result, vector->data, vector->count * sizeof( HilbertHandle ) );
 ...
 */
static inline HilbertHandle * hilbert_ivector_getp( struct IndexVector * vector, size_t index ) {
	assert( vector != NULL );
	assert( index < vector->count );
 ...
 */
static inline void hilbert_ivector_set( struct IndexVector * vector, size_t index, HilbertHandle value ) {
	assert( vector != NULL );
	assert( index < vector->count );
 ...
 * 	returned.
 */
static inline void * hilbert_ivector_iterator_start( const IndexVector * vector ) WARN_UNUSED_RESULT;
static inline void * hilbert_ivector_iterator_start( const IndexVector * vector ) {
	assert( vector != NULL );
 ...
 * 	returned.
 */
static inline void * hilbert_ivector_iterator_end( const IndexVector * vector ) WARN_UNUSED_RESULT;
static inline void * hilbert_ivector_iterator_end( const IndexVector * vector ) {
	assert( vector != NULL );
 ...
 */
static inline HilbertHandle hilbert_ivector_iterator_get( const IndexVector * vector, void * iter ) {
	assert( vector != NULL );
	assert( iter != NULL );

	HilbertHandle * current = iter;
 ...
 */
static inline HilbertHandle * hilbert_ivector_iterator_getp( const IndexVector * vector, void * iter ) {
	assert( vector != NULL );
	assert( iter != NULL );
 ...
 */
static inline void * hilbert_ivector_iterator_next( const IndexVector * vector, void * iter ) WARN_UNUSED_RESULT;
static inline void * hilbert_ivector_iterator_next( const IndexVector * vector, void * iter ) {
	assert( vector != NULL );
	assert( iter != NULL );
 ...
 */
static inline void * hilbert_ivector_iterator_previous( const IndexVector * vector, void * iter ) WARN_UNUSED_RESULT;
static inline void * hilbert_ivector_iterator_previous( const IndexVector * vector, void * iter ) {
	assert( vector != NULL );
	assert( iter != NULL );
...
#ifdef CL_IndexVector_IS_SORTABLE // sort implementation, see http://www.keithschwarz.com/smoothsort/ for details
 ...
 */
static inline void hilbert_ivector_lheap_rebalance( IndexVector * vector, int shift, size_t root ) {
	assert( vector != NULL );
	...
	while ( shift >= 2 ) {
		...
		HilbertHandle tmp = vector->data[root];
 ...
 */
static inline void hilbert_ivector_lheap_rectify( IndexVector * vector, CLLeonardoBitvector lv, int shift, size_t end ) {
	assert( vector != NULL );
	...
	hilbert_ivector_lheap_rebalance( vector, oshift, i );
}
 ...
 * @param vector Pointer to a vector.
 */
static inline void hilbert_ivector_sort( IndexVector * vector ) {
	assert( vector != NULL );
	...
	for ( size_t i = 1; i != vector->count; ++i ) {
		...
		if ( need_full_fix ) {
			hilbert_ivector_lheap_rectify( vector, lv, shift, i );
		} else {
			hilbert_ivector_lheap_rebalance( vector, shift, i );
		}
	}
	...
	for ( size_t i = vector->count; i != 0; --i ) {
		if ( shift <= 1 ) {
			...
			hilbert_ivector_lheap_rectify( vector, lv >> 1, shift + 1, left );
			hilbert_ivector_lheap_rectify( vector, lv, shift, right );
		}
	}
}
 ...
 */
static inline void hilbert_ivector_sort_raw( size_t size, HilbertHandle * array ) {
	assert( ( size == 0 ) || ( array != NULL ) );

	/* Build fake vector */
	IndexVector fake = ( IndexVector ) {
		.count = size,
		.size = size,
		.data = array,
	};

	hilbert_ivector_sort( &fake );
}
 ...
 *
 * @sa #hilbert_ivector_sort()
 */
static inline size_t hilbert_ivector_search( const IndexVector * vector, const HilbertHandle e ) {
	assert( vector != NULL );
 ...
 *
 * @sa #hilbert_ivector_sort_raw()
 */
static inline size_t hilbert_ivector_search_raw( size_t size, const HilbertHandle * array, const HilbertHandle e ) {
	IndexVector fake = ( IndexVector ) {
		.count = size,
		.size = size,
		.data = ( HilbertHandle * ) array // contents will not be altered
	};

	return hilbert_ivector_search( &fake, e );
}
