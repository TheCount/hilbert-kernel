...
static inline ParamMap * hilbert_pmap_new(void) {
	...
	result->buckets = calloc(CL_ParamMap_NUMBUCKETS, sizeof(*result->buckets));
 ...
 */
static inline void hilbert_pmap_del(struct ParamMap * map) {
	assert (map != NULL);
 ...
 */
static inline struct ParamMapBucket * hilbert_pmap_find(struct ParamMapBucket * buckets, size_t sizemask, HilbertHandle key, size_t hash) {
	assert (buckets != NULL);
	assert (sizemask > 0);
 ...
 */
static inline int hilbert_pmap_set(ParamMap * map, HilbertHandle key, HilbertHandle value) {
	assert (map != NULL);

	struct ParamMapEntry entry = { .key = key, .value = value };
	size_t hash = cl_hash32(key);
	struct ParamMapBucket * candidate = hilbert_pmap_find(map->buckets, map->sizemask, key, hash);
	if (candidate->state == CL_ParamMap_BUCKET_OCCUPIED) { /* mapping already present */
	...
	if (newcount > map->threshold) {
		...
		struct ParamMapBucket * newbuckets = calloc(1, newalloc);
		if (newbuckets == NULL)
			return -1;
		for (size_t i = 0; i <= map->sizemask; ++i) {
			if (map->buckets[i].state == CL_ParamMap_BUCKET_OCCUPIED) {
				hilbert_pmap_store(newbuckets, newsizemask, map->buckets[i].entry,
		...
		/* store mapping */
		hilbert_pmap_store(map->buckets, map->sizemask, entry, hash);
		map->count = newcount;
		return 0;
	}
	...
	candidate->state = CL_ParamMap_BUCKET_OCCUPIED;
	map->count = newcount;
	return 0;
}
 ...
 */
static inline HilbertHandle * hilbert_pmap_get(const ParamMap * map, HilbertHandle key) {
	assert (map != NULL);

	struct ParamMapBucket * candidate = hilbert_pmap_find(map->buckets, map->sizemask, key, cl_hash32(key));
 ...
 */
static inline ParamMapIterator hilbert_pmap_iterator_new(ParamMap * map) {
	assert (map != NULL);

	return (ParamMapIterator) { .map = map, .index = 0 };
}
...
static inline int hilbert_pmap_iterator_hasnext(ParamMapIterator * i) {
	...
	for(; i->index <= i->map->sizemask; ++i->index)
		if (i->map->buckets[i->index].state == CL_ParamMap_BUCKET_OCCUPIED)
			return 1;
	return 0;
}
