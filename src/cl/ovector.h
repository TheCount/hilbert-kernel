...
#ifndef CL_ObjectVector_H__
#define CL_ObjectVector_H__
...
#undef COMPARATOR
#define CL_ObjectVector_IS_SORTABLE
#endif
#undef CL_A
#undef CL_B
 ...
 */
static inline int hilbert_ovector_init2( ObjectVector * vector, size_t minsize ) WARN_UNUSED_RESULT;
static inline int hilbert_ovector_init2( ObjectVector * vector, size_t minsize ) {
	assert( vector != NULL );
 ...
 * 	On error, -1 is returned.
 */
static inline int hilbert_ovector_init( ObjectVector * vector ) WARN_UNUSED_RESULT;
static inline int hilbert_ovector_init( ObjectVector * vector ) {
	return hilbert_ovector_init2( vector, 0 );
}
 ...
 */
static inline ObjectVector * hilbert_ovector_new2( size_t minsize ) WARN_UNUSED_RESULT;
static inline ObjectVector * hilbert_ovector_new2( size_t minsize ) {
	ObjectVector * result;
	...
	int rc = hilbert_ovector_init2( result, minsize );
	if ( rc != 0 ) {
		free( result );
		return NULL;
	}
 ...
 * 	On error, <code>NULL</code> is returned.
 */
static inline ObjectVector * hilbert_ovector_new(void) WARN_UNUSED_RESULT;
static inline ObjectVector * hilbert_ovector_new(void) {
	return hilbert_ovector_new2( 0 );
}
 ...
 * @param vector Pointer to vector.
 */
static inline void hilbert_ovector_fini( ObjectVector * vector ) {
	assert( vector != NULL );
 ...
 * 	#hilbert_ovector_fini().
 * 	The returned array must be freed by the caller.
 */
static inline union Object * * hilbert_ovector_dismantle( ObjectVector * vector ) WARN_UNUSED_RESULT;
static inline union Object * * hilbert_ovector_dismantle( ObjectVector * vector ) {
	assert( vector != NULL );

	union Object * * result = realloc( vector->data, vector->count * sizeof( *vector->data ) );
 ...
 */
static inline int hilbert_ovector_pushback(ObjectVector * vector, union Object * elt) WARN_UNUSED_RESULT;
 ...
 * 	If the vector does not have any elements, the behaviour is undefined.
 */
static inline union Object * hilbert_ovector_first( const ObjectVector * vector ) {
	assert( vector != NULL );
	assert( vector->count > 0 );
 ...
 * 	On error, -1 is returned.
 */
static inline int hilbert_ovector_append( ObjectVector * restrict dest, const ObjectVector * restrict src ) {
	assert( dest != NULL );
	assert( src != NULL );
	...
	if ( newsize < dest->size ) {
		...
		size_t newalloc = newsize * sizeof( union Object * );
		if ( newalloc <= sizeof( union Object * ) ) {
			return -1;
		}
		union Object * * newdata = realloc( dest->data, newalloc );
		if ( newdata == NULL ) {
			return -1;
		}
		dest->size = newsize;
	...
	memcpy( dest->data + dest->count, src->data, src->count * sizeof( union Object * ) );
	dest->count = newcount;
...
static inline union Object * * hilbert_ovector_toarray( const ObjectVector * vector ) {
	...
	union Object * * result = malloc( vector->count * sizeof( union Object * ) );
	if ( result == NULL ) {
		return NULL;
	}
	memcpy( result, vector->data, vector->count * sizeof( union Object * ) );
 ...
 */
static inline union Object * * hilbert_ovector_getp( struct ObjectVector * vector, size_t index ) {
	assert( vector != NULL );
	assert( index < vector->count );
 ...
 */
static inline void hilbert_ovector_set( struct ObjectVector * vector, size_t index, union Object * value ) {
	assert( vector != NULL );
	assert( index < vector->count );
 ...
 * 	returned.
 */
static inline void * hilbert_ovector_iterator_start( const ObjectVector * vector ) WARN_UNUSED_RESULT;
static inline void * hilbert_ovector_iterator_start( const ObjectVector * vector ) {
	assert( vector != NULL );
 ...
 * 	returned.
 */
static inline void * hilbert_ovector_iterator_end( const ObjectVector * vector ) WARN_UNUSED_RESULT;
static inline void * hilbert_ovector_iterator_end( const ObjectVector * vector ) {
	assert( vector != NULL );
 ...
 */
static inline union Object * hilbert_ovector_iterator_get( const ObjectVector * vector, void * iter ) {
	assert( vector != NULL );
	assert( iter != NULL );

	union Object * * current = iter;
 ...
 */
static inline union Object * * hilbert_ovector_iterator_getp( const ObjectVector * vector, void * iter ) {
	assert( vector != NULL );
	assert( iter != NULL );
 ...
 */
static inline void * hilbert_ovector_iterator_next( const ObjectVector * vector, void * iter ) WARN_UNUSED_RESULT;
static inline void * hilbert_ovector_iterator_next( const ObjectVector * vector, void * iter ) {
	assert( vector != NULL );
	assert( iter != NULL );
 ...
 */
static inline void * hilbert_ovector_iterator_previous( const ObjectVector * vector, void * iter ) WARN_UNUSED_RESULT;
static inline void * hilbert_ovector_iterator_previous( const ObjectVector * vector, void * iter ) {
	assert( vector != NULL );
	assert( iter != NULL );
...
#ifdef CL_ObjectVector_IS_SORTABLE // sort implementation, see http://www.keithschwarz.com/smoothsort/ for details
 ...
 */
static inline void hilbert_ovector_lheap_rebalance( ObjectVector * vector, int shift, size_t root ) {
	assert( vector != NULL );
	...
	while ( shift >= 2 ) {
		...
		union Object * tmp = vector->data[root];
 ...
 */
static inline void hilbert_ovector_lheap_rectify( ObjectVector * vector, CLLeonardoBitvector lv, int shift, size_t end ) {
	assert( vector != NULL );
	...
	hilbert_ovector_lheap_rebalance( vector, oshift, i );
}
 ...
 * @param vector Pointer to a vector.
 */
static inline void hilbert_ovector_sort( ObjectVector * vector ) {
	assert( vector != NULL );
	...
	for ( size_t i = 1; i != vector->count; ++i ) {
		...
		if ( need_full_fix ) {
			hilbert_ovector_lheap_rectify( vector, lv, shift, i );
		} else {
			hilbert_ovector_lheap_rebalance( vector, shift, i );
		}
	}
	...
	for ( size_t i = vector->count; i != 0; --i ) {
		if ( shift <= 1 ) {
			...
			hilbert_ovector_lheap_rectify( vector, lv >> 1, shift + 1, left );
			hilbert_ovector_lheap_rectify( vector, lv, shift, right );
		}
	}
}
 ...
 */
static inline void hilbert_ovector_sort_raw( size_t size, union Object * * array ) {
	assert( ( size == 0 ) || ( array != NULL ) );

	/* Build fake vector */
	ObjectVector fake = ( ObjectVector ) {
		.count = size,
		.size = size,
		.data = array,
	};

	hilbert_ovector_sort( &fake );
}
 ...
 *
 * @sa #hilbert_ovector_sort()
 */
static inline size_t hilbert_ovector_search( const ObjectVector * vector, const union Object * e ) {
	assert( vector != NULL );
 ...
 *
 * @sa #hilbert_ovector_sort_raw()
 */
static inline size_t hilbert_ovector_search_raw( size_t size, const union Object * * array, const union Object * e ) {
	ObjectVector fake = ( ObjectVector ) {
		.count = size,
		.size = size,
		.data = ( union Object * * ) array // contents will not be altered
	};

	return hilbert_ovector_search( &fake, e );
}
