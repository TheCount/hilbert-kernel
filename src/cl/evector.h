 ...
 * - <code>ExpressionVectorIterator</code>:
 *   Vector iterator type name.
 * - <code>hilbert_evector</code>:
 *   Prefix of function names.
 */

#ifndef HILBERT_CL_ExpressionVector_H__
#define HILBERT_CL_ExpressionVector_H__
 ...
 */
struct ExpressionVectorIterator {
	/**
	 * Pointer to vector over which we are iterating.
	 */
	struct ExpressionVector * vector;
...
typedef struct ExpressionVectorIterator ExpressionVectorIterator;
 ...
 */
static inline int hilbert_evector_grow(ExpressionVector * vector) {
	assert (vector != NULL);
...
static inline int hilbert_evector_pushback(ExpressionVector * vector, struct HilbertExpression * elt) {
	...
	if (newcount > vector->size) {
		if (hilbert_evector_grow(vector) != 0)
			return -1;
	}
 ...
 * 	On error, <code>-1</code> is returned.
 */
static inline int hilbert_evector_append(ExpressionVector * dest, ExpressionVector * src) {
	assert (dest != NULL);
	assert (src != NULL);
	...
	while (newcount > dest->size) {
		if (hilbert_evector_grow(dest) != 0)
...
static inline struct HilbertExpression * * hilbert_evector_toarray(const ExpressionVector * vector) {
	...
	struct HilbertExpression * * result = malloc(allocsize);
	if (result == NULL)
		return NULL;
 ...
 */
static inline ExpressionVectorIterator hilbert_evector_iterator_new(ExpressionVector * vector) {
	assert (vector != NULL);

	return (ExpressionVectorIterator) { .vector = vector, .index = 0 };
}
 ...
 */
static inline int hilbert_evector_iterator_hasnext(ExpressionVectorIterator * i) {
	assert (i != NULL);
 ...
 */
static inline struct HilbertExpression * hilbert_evector_iterator_next(ExpressionVectorIterator * i) {
	assert (i != NULL);
